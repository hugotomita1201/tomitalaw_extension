...
if (typeof window.DS160_INITIALIZED === 'undefined') {
  ...
  class TwoPassFiller {
    ...
    // Batch process all previous education with Add Another functionality
    fillPreviousEducation(data) {
      console.log('Starting batch processing for previous education');

      // Find the Add Another button for education - look for enabled buttons only
      // The ID usually contains "InsertButtonPrevEduc"
      const addButtons = document.querySelectorAll('a[id*="InsertButtonPrevEduc"]');
      let educationAddButton = null;

      // Find the first enabled button (should be ctl00's button)
      for (const btn of addButtons) {
        if (!btn.disabled && !btn.hasAttribute('disabled')) {
          educationAddButton = btn;
          console.log('Found enabled Add Another button for education:', btn.id);
          break;
        }
      }

      if (!educationAddButton) {
        console.log('No enabled Add Another button found for education');
        return false;
      }

      console.log(`Found Add Another button: ${educationAddButton.id}`);

      const allFields = [];

      // Process each education entry
      data.workEducation?.education?.institutions?.forEach((school, index) => {
        if (!school) return;

        const ctl = `ctl${index.toString().padStart(2, '0')}`;
        console.log(`Processing school ${index}: ${school.name}`);

        // Add all fields for this school
        // School Name
        if (school.name) {
          allFields.push({
            id: `ctl00_SiteContentPlaceHolder_FormView1_dtlPrevEduc_${ctl}_tbxSchoolName`,
            value: this.sanitizeEmployerName(school.name)
          });
        }
        ...
        if (school.address?.street1) {
          allFields.push({
            id: `ctl00_SiteContentPlaceHolder_FormView1_dtlPrevEduc_${ctl}_tbxSchoolAddr1`,
            value: this.truncateAddress(school.address.street1)
          });
        }
        if (school.address?.street2) {
          allFields.push({
            id: `ctl00_SiteContentPlaceHolder_FormView1_dtlPrevEduc_${ctl}_tbxSchoolAddr2`,
            value: school.address.street2
          });
        }
        if (school.address?.city) {
          allFields.push({
            id: `ctl00_SiteContentPlaceHolder_FormView1_dtlPrevEduc_${ctl}_tbxSchoolCity`,
            value: school.address.city
          });
        }
        if (school.address?.state) {
          allFields.push({
            id: `ctl00_SiteContentPlaceHolder_FormView1_dtlPrevEduc_${ctl}_tbxEDUC_INST_ADDR_STATE`,
            value: school.address.state
          });
        }
        if (school.address?.postalCode) {
          allFields.push({
            id: `ctl00_SiteContentPlaceHolder_FormView1_dtlPrevEduc_${ctl}_tbxEDUC_INST_POSTAL_CD`,
            value: school.address.postalCode
          });
        }
        if (school.address?.country) {
          allFields.push({
            id: `ctl00_SiteContentPlaceHolder_FormView1_dtlPrevEduc_${ctl}_ddlSchoolCountry`,
            value: this.mapCountry(school.address.country)
          });
        }

        // Course of Study
        if (school.courseOfStudy) {
          allFields.push({
            id: `ctl00_SiteContentPlaceHolder_FormView1_dtlPrevEduc_${ctl}_tbxSchoolCourseOfStudy`,
            value: school.courseOfStudy
          });
        }

        // Start date fields
        if (school.fromDate) {
          const startDay = this.getDayFromDate(school.fromDate);
          const startMonth = this.getMonthNumber(school.fromDate);
          const startYear = this.getYearFromDate(school.fromDate);

          if (startDay) {
            allFields.push({
              id: `ctl00_SiteContentPlaceHolder_FormView1_dtlPrevEduc_${ctl}_ddlSchoolFromDay`,
              value: startDay
            });
          }
          if (startMonth) {
            allFields.push({
              id: `ctl00_SiteContentPlaceHolder_FormView1_dtlPrevEduc_${ctl}_ddlSchoolFromMonth`,
              value: startMonth
            });
          }
          if (startYear) {
            allFields.push({
              id: `ctl00_SiteContentPlaceHolder_FormView1_dtlPrevEduc_${ctl}_tbxSchoolFromYear`,
              value: startYear
            });
          }
        }
        ...
        // End date fields
        if (school.toDate) {
          const endDay = this.getDayFromDate(school.toDate);
          const endMonth = this.getMonthNumber(school.toDate);
          const endYear = this.getYearFromDate(school.toDate);

          if (endDay) {
            allFields.push({
              id: `ctl00_SiteContentPlaceHolder_FormView1_dtlPrevEduc_${ctl}_ddlSchoolToDay`,
              value: endDay
            });
          }
          if (endMonth) {
            allFields.push({
              id: `ctl00_SiteContentPlaceHolder_FormView1_dtlPrevEduc_${ctl}_ddlSchoolToMonth`,
              value: endMonth
            });
          }
          if (endYear) {
            allFields.push({
              id: `ctl00_SiteContentPlaceHolder_FormView1_dtlPrevEduc_${ctl}_tbxSchoolToYear`,
              value: endYear
            });
          }
        }
      ...
      console.log(`Injecting ${validFields.length} fields for ${data.workEducation?.education?.institutions?.length} schools`);

      // Inject script with all fields and the add button
      if (validFields.length > 0) {
        this.injectScript(validFields, educationAddButton.id);
        return true;
      }
  ...
  class DS160Extension {
    ...
    async startFilling() {
      ...
      if (currentPage === 'workEducationPrevious') {
        ...
        // Use batch processing for previous education if we have any
        if (educationCount > 0) {
          const success = this.filler.fillPreviousEducation(this.data);
        }
      }
